...
export class App {
    ...
    async config(io: Server) {
            ...
            if (Object.prototype.hasOwnProperty.call(this.componentNodes, req.params.name)) {
                try {
                    ...
                    for (let i = 0; i < loadFromDbCollections.length; i += 1) {
                        ...
                        else if (loadFromDbCollections[i] === 'User') collection = User
        ...
        // Get all Users
        this.app.get('/api/v1/users', async (req: Request, res: Response) => {
            const users = await this.AppDataSource.getMongoRepository(User).find()
            return res.json(users)
        })

        // Get specific user via id
        this.app.get('/api/v1/users/:id', async (req: Request, res: Response) => {
            try {
                const user = await this.AppDataSource.getMongoRepository(User).findOneBy({
                    _id: new ObjectId(req.params.id)
                })
        ...
        // Create new user
        this.app.post('/api/v1/users', async (req: Request, res: Response) => {
            try {
                const body: IUserRequestBody = req.body
                const { userName} = body
                ...
                Object.assign(newUser, newBody)

                const user = await this.AppDataSource.getMongoRepository(User).create(newUser)
                const results = await this.AppDataSource.getMongoRepository(User).save(user)
                return res.json(results)
            } catch (e) {
                return res.status(500).send(e)
            }
        ...
        // Update user
        this.app.put('/api/v1/users/:id', async (req: Request, res: Response) => {
            const user = await this.AppDataSource.getMongoRepository(User).findOneBy({
                _id: new ObjectId(req.params.id)
            })
            ...
            Object.assign(updateUser, body)

            this.AppDataSource.getMongoRepository(User).merge(user, updateUser)
            try {
                const results = await this.AppDataSource.getMongoRepository(User).save(user)
                return res.json(results)
            } catch (e) {
        ...
        // Delete user via id
        this.app.delete('/api/v1/users/:id', async (req: Request, res: Response) => {
            const deletQuery = {
                _id: new ObjectId(req.params.id)
            } as any
            const results = await this.AppDataSource.getMongoRepository(User).delete(deletQuery)
            return res.json(results)
        })
